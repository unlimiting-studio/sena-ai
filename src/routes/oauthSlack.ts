...
export async function slackOAuthRoutes(fastify: FastifyInstance) {
  fastify.get("/slack/start", async (request, reply) => {
    try {
      const query = request.query as { state?: string; scope?: string };
      const state = query.state?.trim() ?? "";
      if (state.length === 0) {
        sendHtml(reply, 400, {
          title: "잘못된 요청입니다",
          message: "Slack 연동 토큰(state)이 필요합니다.",
          variant: "error",
        });
        return;
      }

      const payload = parseSlackLinkToken(state);
      ...
      if (!CONFIG.SLACK_CLIENT_ID) {
        sendHtml(reply, 500, {
          title: "환경 구성이 필요합니다",
          message: "Slack OAuth 클라이언트 정보가 설정되어 있지 않아 연동을 진행할 수 없습니다.",
          variant: "error",
        });
        return;
      }

      const isSearchScope = query.scope === "search";

      const authorizeUrl = new URL("https://slack.com/oauth/v2/authorize");
      authorizeUrl.searchParams.set("client_id", CONFIG.SLACK_CLIENT_ID);
      authorizeUrl.searchParams.set("redirect_uri", buildSlackOAuthCallbackUrl());
      authorizeUrl.searchParams.set("user_scope", isSearchScope ? "search:read" : "identity.basic,identity.email");
      authorizeUrl.searchParams.set("state", state);

      reply.redirect(authorizeUrl.toString());
    } catch (error) {
      const message = error instanceof Error ? error.message : "인증을 시작하는 중 오류가 발생했습니다.";
      sendHtml(reply, 400, {
        title: "잘못된 요청입니다",
        message,
        variant: "error",
  ...
  fastify.get("/slack/callback", async (request, reply) => {
    ...
    let parsedQuery: z.infer<typeof OAuthCallbackQuerySchema>;
    try {
      parsedQuery = OAuthCallbackQuerySchema.parse(request.query);
    } catch (error) {
      const message = error instanceof Error ? error.message : "Slack OAuth 응답을 해석할 수 없습니다.";
      sendHtml(reply, 400, {
        title: "잘못된 요청입니다",
        message,
        variant: "error",
    ...
    if (parsedQuery.error) {
      sendHtml(reply, 400, {
        title: "Slack 연동이 취소되었습니다",
        message: parsedQuery.error_description ?? "사용자가 Slack OAuth를 취소했습니다.",
        variant: "info",
      });
      return;
    }

    let slackLinkTokenPayload: ReturnType<typeof parseSlackLinkToken>;
    try {
      slackLinkTokenPayload = parseSlackLinkToken(parsedQuery.state);
      ...
      sendHtml(reply, 400, {
        title: "잘못된 요청입니다",
        message: "Slack 연동 토큰을 확인할 수 없습니다.",
        variant: "error",
      });
      return;
    }
    ...
    try {
      const tokenResponse = await fetch("https://slack.com/api/oauth.v2.access", {
        ...
        body: new URLSearchParams({
          ...
          redirect_uri: buildSlackOAuthCallbackUrl(),
        }),
      });
      ...
      if (!tokenData.ok || !tokenData.authed_user?.access_token) {
        throw new Error(tokenData.error ?? "Slack OAuth 토큰 발급에 실패했습니다.");
      }
      ...
      let resumed = false;
      if (slackLinkTokenPayload.sessionId) {
        resumed = await SlackClaudeAgent.instance
          .resumeSessionFromLinkToken({
            sessionId: slackLinkTokenPayload.sessionId,
            slack: {
              teamId: slackLinkTokenPayload.teamId,
              channelId: slackLinkTokenPayload.channelId,
              threadTs: slackLinkTokenPayload.threadTs,
              messageTs: slackLinkTokenPayload.messageTs,
              slackUserId,
            },
            provider: "slack",
            continuationText,
      ...
      const message = error instanceof Error ? error.message : "Slack OAuth 처리 중 오류가 발생했습니다.";
      sendHtml(reply, 500, {
        title: "Slack 연동에 실패했습니다",
        message: "Slack에서 토큰을 발급받는 과정에서 문제가 발생했습니다.",
        detail: message,
        variant: "error",
      });
    }
