...
- Main Flow:
  ...
  3. 각 스케줄 실행은 `trigger: 'schedule'` 턴으로 위임한다. 스케줄에 `disabledTools`가 설정돼 있으면 턴에 함께 전달한다.
...
- Schedule 옵션:
  `name`, `prompt`, `timezone`, `disabledTools?: string[]`
- 반환:
...
- Given `disabledTools: ['Bash', 'Write']`가 설정된 heartbeat 스케줄, When 턴이 실행되면, Then 해당 턴의 `ProcessTurnOptions.disabledTools`에 `['Bash', 'Write']`가 전달된다.
- Given `disabledTools`가 생략된 스케줄, When 턴이 실행되면, Then `disabledTools`는 `undefined`이고 도구 제한 없이 실행된다.
## 개편 메모
