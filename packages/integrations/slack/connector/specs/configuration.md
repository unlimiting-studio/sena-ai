...
- Main Flow:
  1. 작성자는 `mention`, `thread`, `channel`, `message`, `reactions` 규칙을 선언한다.
...
- Actor:
  Slack connector
- Trigger:
  하나의 Slack message event가 `mention`, `thread`, `channel`, `message` 후보를 동시에 만족함
- Preconditions:
  - 4개 trigger 중 둘 이상이 설정상 활성이다.
- Main Flow:
  1. connector는 해당 raw 이벤트에서 message trigger 후보 목록을 만든다.
  2. 고정 우선순위 `mention > thread > channel > message`에 따라 후보를 순서대로 평가한다.
...
- Category:
  Contract
- Description:
  메시지 계열 trigger 우선순위는 `mention > thread > channel > message`로 고정되며 사용자 설정으로 바꾸지 않는다.
- Scope:
  `SLACK-CONFIG-02`
- Verification:
...
- Category:
  Safety
- Description:
  `channel`과 `message` trigger는 기본 비활성 상태여야 하며, 명시적 opt-in(`channel` 또는 `message` key 선언) 없이는 최상위 채널 메시지를 turn으로 만들면 안 된다.
- Scope:
  전체
- Verification:
...
- 고정 순서 `mention > thread > channel > message`로 선택한다.
...
- `message` 후보는 채널 메시지(thread_ts 없음)와 쓰레드 메시지(봇 참여 여부 무관) 모두를 대상으로 한다. `thread` key가 활성이면 봇 참여 스레드는 우선순위에서 `thread`가 먼저 선택되므로 중복 실행은 없다. `thread` key가 비활성이면 봇 참여 스레드의 후속 답글도 `message`가 처리한다.
