...
- Main Flow:
  ...
  4. invalid reaction rule 형식이면 시작 시점에 오류를 낸다.
- Outputs:
...
- Preconditions:
  - 정규화된 `SlackTriggerEvent`를 만들 수 있다.
- Main Flow:
  1. connector는 `SlackTriggerEvent`를 만든다.
  2. filter를 호출한다.
...
- Main Flow:
  ...
  2. rule.filter가 있으면 먼저 평가한다.
  3. rule이 `{ action: 'abort' }`면 대상 conversation을 중단한다.
  4. rule이 prompt source면 reaction 전용 turn을 제출한다.
- Alternative Flow:
...
- Category:
  Shape
- Description:
  `triggers` 객체가 존재할 때 key가 없는 trigger는 비활성으로 해석해야 하며 `false` 같은 별도 disable sentinel은 요구하지 않는다.
- Scope:
  `SLACK-CONFIG-01`
- Verification:
...
- filter 인자는 Slack 메시지 기준 정보(`channelId`, `userId`, `text`, `ts`, `threadTs`, `reaction`, `raw` 등)를 담은 `SlackTriggerEvent`다.
