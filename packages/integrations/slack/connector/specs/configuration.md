...
- Main Flow:
  ...
  3. invalid reaction rule이면 시작 시점에 오류를 낸다.
- Alternative Flow:
...
- Main Flow:
  ...
  2. 고정 우선순위 `mention > thread > channel`에 따라 가장 높은 enabled trigger를 선택한다.
...
- Scope:
  `SLACK-CONFIG-03`
- Measurement:
  동일 상대 경로에 대한 base directory resolution 결과
- Verification:
...
- Observability Plan:
  - selected trigger kind, skipped trigger kinds, prompt source type, reaction rule action, resolved prompt base directory를 debug log로 남긴다.
- Migration / Rollback:
