...
- Main Flow:
  1. trigger 필드 자체가 function이면:
     ...
     - 유효한 반환값의 shape은 다음 disjoint union이다:
       ...
       - `{ text: string; thinkingMessage?: string | false; disabledTools?: string[] }` — inline prompt 명시형.
       - `{ file: string; thinkingMessage?: string | false; disabledTools?: string[] }` — 파일 참조 prompt.
       - `{ abort: true }` — (reaction 전용) abort action. abort 경로에서는 `ConnectorOutput`을 생성하지 않으므로 `thinkingMessage`와 `disabledTools`는 허용하지 않는다.
...
### Constraint ID: SLACK-CONFIG-CON-009 (disabledTools)

- Category:
  Contract
- Description:
  trigger function이 반환한 `disabledTools`는 `InboundEvent.disabledTools`에 그대로 주입되어야 한다. connector가 목록을 가공하거나 필터링해서는 안 된다. `string` 반환 및 `{ abort: true }` 반환에서는 `disabledTools`를 허용하지 않는다.
- Scope:
  `SLACK-CONFIG-03`, `SLACK-CONFIG-04`
- Verification:
  disabledTools injection unit test

### Constraint ID: SLACK-CONFIG-CON-010
...
type SlackMessageTriggerFunctionResult =
  | string                                          // inline prompt
  | { text: string; thinkingMessage?: string | false; disabledTools?: string[] }
  | { file: string; thinkingMessage?: string | false; disabledTools?: string[] }
...
- `function`
  ...
  - `{ text: string; thinkingMessage?: string | false; disabledTools?: string[] }` 반환 시 inline prompt 명시형으로 사용한다.
  - `{ file: string; thinkingMessage?: string | false; disabledTools?: string[] }` 반환 시 파일 참조로 해석한다.
  - `{ abort: true }` 반환 시 (reaction 전용) abort action으로 처리한다. abort 경로에서는 `thinkingMessage`와 `disabledTools`를 허용하지 않는다.
  - 위 케이스 외의 shape은 유효하지 않으며 해당 이벤트를 실패 처리한다.
...
#### disabledTools Resolution Rules

- trigger function의 반환값이 객체(`{ text }` 또는 `{ file }`)이고 `disabledTools?: string[]`를 포함하면, connector는 이를 `InboundEvent.disabledTools`에 주입한다.
- `disabledTools`가 없거나 `undefined`이면 도구 제한 없이 실행한다.
- `string` 반환(inline prompt shorthand)에서는 `disabledTools`를 지정할 수 없다. 도구 제한이 필요하면 객체 형태를 사용해야 한다.
- `{ abort: true }` 반환에서는 `disabledTools`를 허용하지 않는다 (turn을 만들지 않으므로).
- 기존 파이프라인(engine → runtime)이 `InboundEvent.disabledTools`를 그대로 소비하므로 connector는 주입만 하면 된다.
...
- Given `channel: (event) => ({ file: './safe.md', disabledTools: ['Bash', 'Write'] })`일 때 When 채널 메시지가 들어오면 Then `InboundEvent.disabledTools`에 `['Bash', 'Write']`가 주입된다.
- Given trigger function이 `disabledTools` 없이 `{ text: '...' }`를 반환할 때 When 이벤트가 들어오면 Then `InboundEvent.disabledTools`는 `undefined`이고 도구 제한 없이 실행된다.
- Given trigger function이 `string`을 반환할 때 When 이벤트가 들어오면 Then `disabledTools`를 지정할 방법이 없으므로 도구 제한 없이 실행된다.
