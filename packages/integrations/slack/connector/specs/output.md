...
ConnectorOutput은 에이전트 일반 출력을 Slack text streaming으로 우선 렌더링하고, final safe payload가 필요할 때만 같은 메시지를 최종 형태로 다시 정리한다.
...
- Main Flow:
  ...
  - final 결과에 table block 같은 safe payload rewrite가 필요하면, 완료된 stream 메시지를 같은 `ts`에서 `chat.update`로 최종 형태로 다시 정리한다.
...
- 렌더링:
  ...
  - 최종 결과는 필요 시 공용 Slack Markdown 패키지의 safe payload로 다시 정리한다.
...
- Given final 결과에 table block 같은 safe payload rewrite가 필요할 때 When stream이 끝나면 Then 같은 ts가 final payload로 다시 정리된다.
