...
- **`project-setup.ko.md`** - 새 프로젝트 초기화, Slack 커넥터 연결(HTTP/Socket Mode), .env 설정, CLI 명령어, 아키텍처, 트러블슈팅
- **`config-guide.ko.md`** - sena.config.ts 필드 레퍼런스, env(), 런타임(permissionMode, allowedTools), 라이프사이클 훅(fileContext, traceLogger, 커스텀), 크론잡/하트비트 스케줄, TurnContext, 공통 패턴
- **`tools-and-mcp.ko.md`** - defineTool로 인라인 도구 작성, slackTools 빌트인 도구, MCP 서버(HTTP/stdio) 연결, disabledTools 턴별 도구 제한, 커스텀 커넥터에서 도구 제어
