...
위 §"PoC 0단계"가 이 영역들을 닫는다. 0단계 결과로 SPEC을 rev. 2로 올린다.

## 확정된 결정 (2026-05-10)

차니 결정으로 다음 4개가 닫혔다. 5번은 본 마이그 종반에 결정.

1. **분담** ✅ — **세나가 구현, 브렌이 검토.**
2. **chat-sdk state adapter** ✅ — **`@chat-adapter/state-pg` (가설)**. 단, 본 마이그 전에 PoC 0단계에서 실제 동작 검증 후 확정.
3. **프로세스 구조** ⏸ — **PoC 0단계에서 ai-sdk + chat-sdk + claude-code/codex-cli provider 조합이 어떻게 굴러가는지 본 다음 결정.** 가설(단일 / 분리 / 다른 안)을 미리 박지 않는다.
4. **앱 자체 패키지명** ✅ — **`@sena-ai/app`**.
5. **v2 모노레포 history 보존 vs orphan branch 완전 reset** — 본 마이그 6/11 deprecation 시점에 결정. 그 전엔 main / v3 두 라인 공존.

## PoC 0단계 — 미지수 닫기

차니의 *"이 조합이 어떻게 작동할지 보고 생각해줘"* 시그널에 따라 **본 마이그(`migration.md` §1~8) 진입 전에 0단계 PoC가 들어간다.** 0단계에서 닫는 미지수는 다음과 같다 (`docs/specs/migration.md` §0 상세):
...
- Zod inline tool이 chat-sdk · ai-sdk 자체 메커니즘으로 흡수되는지 (claude-code provider가 AI SDK Zod tool 미지원)
- chat-sdk 자체 observability 범위 (`traceLogger`를 어느 레이어에 둘지)
- **`@chat-adapter/state-pg` 실 동작** (확정 결정 #2 검증)
- **프로세스 구조 결정 입력** — 단일 프로세스로도 hot-reload / restart / 다중 connector가 깨지지 않는지 (확정 결정 #3 입력)

0단계 결과 보고 후에야 본 마이그 일정(PRD §10)이 본격 시작된다.
