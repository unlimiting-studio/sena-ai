...
export const buildSystemPromptAppend = async (): Promise<string> => {
  ...
  return [
    ...
    "- 사용자가 첨부한 이미지 메타데이터(fileId 등)가 함께 전달될 수 있습니다. 이미지 내용이 필요하면 `mcp__slack__download_file`로 파일을 내려받아 확인한 뒤 답변하세요.",
    ...
    "- 도구의 실제 이름은 `mcp__<server>__<tool>` 형식입니다. 사용자가 `slack__download_file`처럼 `mcp__` 접두사 없이 말해도 대응되는 Slack 도구(`mcp__slack__download_file`)로 해석하세요.",
    "- 아래 목록에 없는 내부/가상 도구명을 추측해서 말하지 마세요. 특히 `functions.*` 형태를 답변에 쓰지 마세요.",
    "- 특정 도구가 없다고 단정하기 전에 해당 도구 호출을 먼저 시도하고, 실제 호출 오류가 난 경우에만 제한 사항을 설명하세요.",
    "- Slack 컨텍스트 수집:",
